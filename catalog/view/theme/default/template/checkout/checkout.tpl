...
<div class="container main-container headerOffset">        
	...
	<div class="row box checkout_form">
	    ...
	    <div class="col-md-6">
			<div class="shiptobilling clearfix">
			    ...
			    <label class="checkbox">
				    <input type="checkbox" name="shipping_address" checked="checked" value="new" <?php if (isset($address_id)) echo 'checked="checked"';?> onclick="jQuery('.shipping-address').toggle()"><?php echo $text_address_new; ?>
			    </label>
				<?php } ?>
		    </div>
			...
			<select name="shipping_address_id" class="form-control shipping_address_id_cls" onchange="getdelivery_address(this.value);">
			  <?php foreach ($addresses as $address) {
					
					$selected_delivery_address = "";
					if(isset($new_shipping_address_id))
					{
							$selected_delivery_address = $new_shipping_address_id;
					}
					?>
			  <?php if (isset($shipping_address_id) && $address['address_id'] == $shipping_address_id) { ?>
				  <option value="<?php echo $address['address_id']; ?>" <?php if($selected_delivery_address == $address['address_id']) { ?> selected="selected" <?php } ?>><?php echo $address['firstname']; ?> <?php echo $address['lastname']; ?>, <?php echo $address['address_1']; ?>, <?php echo $address['city']; ?>, <?php echo $address['zone']; ?>, <?php echo $address['country']; ?></option>
				  <?php } else { ?>
				  <option value="<?php echo $address['address_id']; ?>" <?php if($selected_delivery_address == $address['address_id']) { ?> selected="selected" <?php } ?>><?php echo $address['firstname']; ?> <?php echo $address['lastname']; ?>, <?php echo $address['address_1']; ?>, <?php echo $address['city']; ?>, <?php echo $address['zone']; ?>, <?php echo $address['country']; ?></option>
				  <?php } ?>
			<?php } ?>
		   ...
		   <?php //}?>
			...
			<div class="form-group required">
			  ...
			  <div class="">
			    <select name="shipping_country_id" id="input-shipping-country" class="form-control" onchange="get_shipping_address();">
			      <option value=""><?php echo $text_select; ?></option>
			      <?php foreach ($countries as $country) { ?>
			      <?php if ($country['country_id'] == $country_id) { ?>
			      <option value="<?php echo $country['country_id']; ?>" selected="selected"><?php echo $country['name']; ?></option>
			...
			<div class="form-group required">
			  ...
			  <div class="">
			    <select name="shipping_zone_id" id="input-shipping-zone" class="form-control"  onchange="get_shipping_address();">
			    </select>
			  </div>
			</div>
		...
		<div class="your_order">
			...
			<img src="<?php echo SITE_URL; ?>image/loader.gif" id="method_loader" style="display: none">
...
<script type="text/javascript"><!--
	...
	$(document).ready(function(){
	   var country = $('#input-shipping-country').val();
	   get_shipping_address();
	   $.ajax({
	        url: 'index.php?route=checkout/checkout/country&country_id=' + country,
	        dataType: 'json',
	        beforeSend: function() {
				$('select[name=\'country_id\']').after(' <i class="fa fa-spinner fa-spin"></i>');
	        },
	...
	function getdelivery_address(address_id)
	{
			window.location = "<?php echo SITE_URL; ?>index.php?route=checkout/checkout/add_shipping_address&address_id="+address_id;
	}
	function get_shipping_address()
	{
			var first_name = $('#input-shipping-firstname').val();
			var last_name = $('#input-shipping-lastname').val();
			var company = $('#input-shipping-company').val();
			var address1 = $('#input-shipping-address-1').val();
			var address2 = $('#input-shipping-address-2').val();
			var city = $('#input-shipping-city').val();
			var postcode = $('#input-shipping-postcode').val();
			var country = $('#input-shipping-country').val();
			var zone = $('#input-shipping-zone').val();
			
			$.ajax({
	        url: 'index.php?route=checkout/checkout/add_delivery_address',
	        type: 'post',
	        data: {first_name:first_name,last_name:last_name,company:company,address1:address1,address2:address2,city:city,postcode:postcode,country:country,zone:zone},
	        success: function(msg) 
	        {
					if (msg == 1) {
							//window.location = "<?php echo SITE_URL; ?>index.php?route=checkout/checkout";
					}
				//jQuery(".payment").html(html);
