<?php
class ModelMessageMessaging extends Model {
...
// ****************************************************************** My Code *********************************************************	
	...
	public function getFeautureProComic() {
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)  WHERE p.role = 'admin' AND p.customer_id = 1 AND p.feature = 1 AND p.status = 1 AND p.posted_by = 1 ORDER BY p.sort_order DESC ";
	...
	public function getFeautureMarket() {
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "grade G ON (G.id = p.grade)  WHERE p.feature = 1 AND p.status = 1 AND p.posted_by = 0 ORDER BY p.sort_order DESC";
