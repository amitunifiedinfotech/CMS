...
class ControllerCheckoutRegister extends Controller {
	...
	public function save() {
		...
		if(isset($this->request->post['confirm_pass'])){
			$this->request->post['confirm'] = $this->request->post['confirm_pass'];
		}
		...
		if (!$json) {
			...
			if ((utf8_strlen(trim($this->request->post['username'])) < 1)) {
				$json['error']['username'] = 'Please enter username!';
			}
			$flag = $this->model_account_customer->DupliUsername($this->request->post['username']);
			if($flag)
				$json['error']['username'] = 'Username is already exists!';
			
			//echo utf8_strlen($this->request->post['dob']);exit;
			if (utf8_strlen($this->request->post['dob']) < 1)
			{
				$json['error']['warning'] = $json['error']['dob'] = 'please enter date of birth!';
			}
		...
		if (!$json) {
			...
			// Update status
			$this->model_account_customer->UpdateStatusCustomer($customer_id);
