...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		//if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			$data['heading_title'] = 'Shopping Cart ';
			...
			foreach ($total_data as $total) {
				$data['totals'][] = array(
					'title' => $total['title'],
					'code' => $total['code'],
					'text'  => ($total['code']=="coupon")?$total['value']:$this->currency->format($total['value'])
				);
			}
	...
	public function add() {
		...
		$this->session->data['price_type'][$product_id] = "normal";
		
		$this->load->model('catalog/product');

		$product_info = $this->model_catalog_product->getProduct($product_id);
		//print_r($product_info);echo 1;exit;
		if ($product_info) {
			...
			if (!$json) {
				...
				$json['total_mob'] = 'Cart ('.$this->currency->format($total).')';
			} else {
	...
	public function cart_add() {
		...
		$this->session->data['price_type'][$product_id] = "grading";
