...
class ControllerCheckoutCheckout extends Controller { 
	...
	public function index() 
	{
		if(!$this->customer->isLogged()) {
			$this->session->data['page_reffer'] = $this->url->link('checkout/checkout');
			$this->response->redirect($this->url->link('account/login'));
		}
		$data = array();
		
		if(isset($this->session->data['new_shipping_address_id']))
		{
			$data['new_shipping_address_id'] = $this->session->data['new_shipping_address_id'];
		}
	...
	public function add_shipping_address()
	{
		...
		$address = $this->model_account_address->getAddress($_GET['address_id']);
		
		$shipping_address = array('country_id' => $address['country_id'], 'zone_id' => $address['zone_id'], 'firstname' => $address['firstname'], 'lastname' => $address['lastname'], 'company' => $address['company'], 'address_1' => $address['address_1']);
		
		$this->session->data['shipping_address'] = $shipping_address;
		$this->session->data['shipping_address_id'] = $_GET['address_id'];
		$this->session->data['new_shipping_address_id'] = $_GET['address_id'];
	...
	public function add_delivery_address()
	{
		...
		$shipping_address = array('country_id' => $_POST['country'], 'zone_id' => $_POST['zone'], 'firstname' => $_POST['first_name'], 'lastname' => $_POST['last_name'], 'company' => $_POST['company'], 'address_1' => $_POST['address1']);
		$this->session->data['shipping_address'] = $shipping_address;
		echo "1";
		die;
	}
		...
		if ($opencart2) 
		{
			...
			if (isset($_POST) && !empty($_POST))
			{
				...
				if ((isset($payment_address) && is_array($payment_address)) || isset($this->session->data['payment_address']))
				{
						...
						if($this->request->post['payment_address_id']!="")
						{
							if($this->request->post['payment_address']!="new")
							{
								...
								$order_data['payment_firstname'] = isset($payment_address['firstname'])?$payment_address['firstname']:$this->request->post['firstname'];
								$order_data['payment_lastname'] = isset($payment_address['lastname'])?$payment_address['lastname']:$this->request->post['lastname'];
								$order_data['payment_company'] = isset($payment_address['company'])?$payment_address['company']:$this->request->post['company'];
								...
								$order_data['payment_address_1'] = isset($payment_address['address_1'])?$payment_address['address_1']:$this->request->post['address_1'];
								$order_data['payment_address_2'] = isset($payment_address['address_2'])?$payment_address['address_2']:$this->request->post['address_2'];
								$order_data['payment_city'] = isset($payment_address['city'])?$payment_address['city']:$this->request->post['city'];
								$order_data['payment_postcode'] = isset($payment_address['postcode'])?$payment_address['postcode']:$this->request->post['postcode'];
								$order_data['payment_zone'] = isset($payment_address['zone'])?$payment_address['zone']:$this->request->post['zone'];
								$order_data['payment_zone_id'] = isset($payment_address['zone_id'])?$payment_address['zone_id']:$this->request->post['zone_id'];
								$order_data['payment_country'] = isset($payment_address['country'])?$payment_address['country']:$this->request->post['country'];
								$order_data['payment_country_id'] = isset($payment_address['country_id'])?$payment_address['country_id']:$this->request->post['country_id'];
								$order_data['payment_address_format'] = isset($payment_address['address_format'])?$payment_address['address_format']:$this->request->post['address_format'];
							...
							else
							{
								...
								$this->load->model('localisation/country');
								$this->load->model('localisation/zone');
								$country_info1 = $this->model_localisation_country->getCountry($this->request->post['country_id']);
								$zone_info1 = $this->model_localisation_zone->getZone($this->request->post['zone_id']);
								
								$order_data['payment_zone'] = $zone_info1['name'];
								$order_data['payment_zone_id'] = $this->request->post['zone_id'];
								$order_data['payment_country'] = $country_info1['name'];
				...
				if (isset($this->session->data['payment_method']['title'])) {
					...
					$order_data['payment_method'] = $this->request->post['_payment_method'];
				}
				...
				if (isset($this->session->data['payment_method']['code'])) {
					...
					$order_data['payment_code'] = $this->request->post['country'];
				}
				...
				if ($this->cart->hasShipping()) {
					...
					if (isset($this->request->post['shipping_firstname']))
					{
						...
						else
						{
							...
							else if(($this->request->post['shipping_address_id']!=""))
							{
			...
			//echo 'aa==';
			//echo $data['payment'] = $this->load->controller('payment/pp_pro');
			//print_r($this->session->data['payment_method']);
			if ($render !== false)
			{
				echo $data['payment'] = $this->load->controller('payment/' . $this->request->post['payment_method']);
			}
		} else
		{
