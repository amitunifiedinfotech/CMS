...
class ControllerToolXLSImport extends Controller {
  ...
  public function index() {
    ...
    $data['breadcrumbs'][] = array(
      'text' => 'My Product',
      'href' => $this->url->link('seller_product/product_list','', 'SSL'),
      'separator' => ' :: '
    );

// link for form action
    $data['action'] = $this->url->link('tool/xls_import', '', 'SSL');
...
// LOGIC
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) { //&& ($this->validate())
	  ...
	  $this->unzip($this->request->files['uploadIMAGEZIP']['tmp_name'], "./image/catalog/", true, false);

	  $this->session->data['success'] = $this->language->get('text_success');
          $this->response->redirect($this->url->link('tool/xls_import', '', 'SSL'));
        }
        else {
...
//----------------Standard controller part--------------------------------------
    ...
    $this->response->setOutput($this->load->view('default/template/tool/xls_import.tpl', $data));
