...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['tracking_number'] = "Tracking Number";
			$data['shipping_carrier'] = "Shipping Carrier";
            ...
            if(($order_info['order_status_id'] == 2) || ($order_info['order_status_id'] == ""))
			{
			    $order_statuses_arr[0] = $order_info['order_status_id'];
			    $order_statuses_arr[1] = 3;
			}
			if($order_info['order_status_id'] == 3)
			{
			    $order_statuses_arr[1] = 5;
			}
			$order_status_list = implode(',',$order_statuses_arr);
			
			
			$order_statuses_arr  = $this->model_localisation_order_status->getSelectedOrderStatuses($order_status_list);
			//echo "<pre>"; print_r($order_statuses); die;
			$data['order_statuses_arr'] = $order_statuses_arr ;
	...
	function order_list()
	{
	   ...
	   $this->load->language('common/dashboard');
		...
		$this->document->setTitle("Order List");

		$data['heading_title'] = $this->language->get('Order List');
		...
		$data['breadcrumbs'][] = array(
			'text' => "Order List",
			'href' => $this->url->link('sale/order/order_list', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		$results = $this->model_sale_order->getOrders($filter_data);
		$post_total = $this->model_sale_order->getOrdersToT();
		...
		$this->response->setOutput($this->load->view('sale/all_order_list.tpl', $data));
	}
}