<?php   
class ControllerNewsletterLatestnews extends Controller {   
	public function index() {
		...
		$post_total= $this->model_latestnews_addedit->getTotalPost();
		$data['post_arr'] = $this->model_latestnews_addedit->getAllPost($filter_data);
		...
		$this->response->setOutput($this->load->view('newsletter/latest_news.tpl', $data));
	}
	...
	public function add_latest_news()
        {
            $data['token'] = $this->session->data['token'];
			$this->load->model('latestnews/addedit');
			$this->document->setTitle("Add Latest News");
			...
			$data['breadcrumbs'][] = array(
				'text'      => "News List",
				'href'      => $this->url->link('newsletter/latestnews', 'token=' . $this->session->data['token'], 'SSL'),
				'separator' => false
			);
		$data['cancel'] = $this->url->link('newsletter/latestnews', 'token=' . $this->session->data['token'] , 'SSL');		
		$data['action'] = $this->url->link('newsletter/latestnews/add_latest_news', 'token=' . $this->session->data['token'] , 'SSL');		
			
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				$this->data = $_POST;
				$this->model_latestnews_addedit->addNews($this->data);
				
				$this->session->data['success'] = 'Latest news added sucessfully';
				$this->response->redirect($this->url->link('newsletter/latestnews', 'token=' . $this->session->data['token'], 'SSL'));
			}
			
			$this->response->setOutput($this->load->view('newsletter/add_latest_news.tpl', $data));
			
           
        }
        
        
        public function edit_latest_news()
        {
		
			$data['token'] = $this->session->data['token'];
			$this->load->model('latestnews/addedit');
			$this->document->setTitle("Edit Latest News");
		...
		$data['action'] = $this->url->link('newsletter/latestnews/add_latest_news', 'token=' . $this->session->data['token'] , 'SSL');			
			...
			$post_arr = $this->model_latestnews_addedit->getNews($_GET['post_id']);
			$data['post_arr'] = $post_arr;
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				$this->data = $_POST;
				$this->model_latestnews_addedit->editNews($_GET['post_id'],$this->data);
				
				$this->session->data['success'] = 'Latest news edited sucessfully';
				$this->response->redirect($this->url->link('newsletter/latestnews', 'token=' . $this->session->data['token'], 'SSL'));
			}
			
			$this->response->setOutput($this->load->view('newsletter/edit_latest_news.tpl', $data));
...
//        }
	...
	//	
	//	if($id!=0){
	//		$this->model_latestnews_addedit->deleteNewsletterTemplate($id);
	...
	//}
	//
	public function del_latest_news()
	{
		$this->load->model('latestnews/addedit');
		...
		if($id!=0){
			$this->model_latestnews_addedit->deleteLatestNews($id);
			$this->session->data['success'] = "Latest News deleted sucessfully!!!";
		}
