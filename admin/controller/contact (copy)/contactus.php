...
class ControllerContactContactus extends Controller {
	...
	public function index() {
		...
		$contact_query_total= $this->model_contact_contact->getTotalContactQuery();
		$data['results_set'] = $this->model_contact_contact->getContactQuery($filter_data);
		//print_r($data['results_set']);
		
		$pagination = new Pagination();
		$pagination->total = $contact_query_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('contact/contactus', 'token=' . $this->session->data['token']. '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($contact_query_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($contact_query_total - $this->config->get('config_limit_admin'))) ? $contact_query_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $contact_query_total, ceil($contact_query_total / $this->config->get('config_limit_admin')));

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('contact/user_contactus.tpl', $data));
	}
	...
	protected function getList() {
		...
		$this->response->setOutput($this->load->view('common/user_contactus.tpl', $data));
	}
