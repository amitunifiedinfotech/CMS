...
class ModelContactContact extends Model {
	...
	public function getContactQuery($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "contactus WHERE reply_id =  0 ORDER BY contact_id DESC";
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "contactus WHERE reply_id =  0");
			return $query->rows;
		}
	}
	
	public function getTotalContactQuery() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "contactus WHERE reply_id =  0");
